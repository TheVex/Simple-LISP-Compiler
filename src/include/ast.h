...
inline Node *make_int_node(long long v) {
  Node *n = new Node(NodeType::NT_INT);
  n->ival = v;
  return n;
}
inline Node *make_real_node(double v) {
  Node *n = new Node(NodeType::NT_REAL);
  n->dval = v;
  return n;
}
inline Node *make_bool_node(bool v) {
  Node *n = new Node(NodeType::NT_BOOL);
  n->bval = v;
  return n;
}
inline Node *make_null_node() { return new Node(NodeType::NT_NULL); }
inline Node *make_ident_node(const std::string &s) {
  Node *n = new Node(NodeType::NT_IDENT);
  n->str = s;
  return n;
}
inline Node *make_string_node(const std::string &s) {
  Node *n = new Node(NodeType::NT_STRING);
  n->str = s;
  return n;
}
...
inline void print_node(Node *n, int depth = 0) {
  ...
  case NodeType::NT_REAL:
    std::cout << "REAL(" << n->dval << ")\n";
    break;
  case NodeType::NT_BOOL:
    std::cout << "BOOL(" << (n->bval ? "true" : "false") << ")\n";
    break;
  case NodeType::NT_NULL:
    std::cout << "NULL\n";
    break;
  case NodeType::NT_IDENT:
    std::cout << "IDENT(" << n->str << ")\n";
    break;
  case NodeType::NT_STRING:
    std::cout << "STRING(" << n->str << ")\n";
    break;
